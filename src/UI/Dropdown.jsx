...
const Dropdown = ({ name, values, onChange, isOpen, onToggle, selectedValue }) => {
  return (
    <div className="relative  text-sm z-50 ">
      ...
      <ul
        className={`absolute max-h-96 py-2 overflow-y-auto w-full h-auto bg-white border-2 border-gray-300 drop-shadow-lg left-0 rounded-lg transition-all duration-300 ease-in-out ${
          isOpen
      ...
      >
        {values.map((value, index) => {
          return(
            <div>
              {value.name && <div className="px-1 py-1 text-xs text-blue opacity-100 font-semibold">{value.name}</div>}
