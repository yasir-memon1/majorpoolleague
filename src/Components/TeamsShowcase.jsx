        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <ul className="col-span-1 px-4 gap-4">
                {column1.map((team, index) => (
                  <li
                    key={index}
                    className={`${ (index !== 0) && "mt-4" } border-[1px] py-4 gap-4  font-semibold text-base text-gray-700  flex justify-center items-center`}
                  >
                    <div className="w-20 h-20 rounded-full overflow-hidden">
                    <img className="object-cover" src={team.image}/>
                    </div>
                    <div className="flex flex-1 flex-col">
                    <Link  className="text-xl  hover:text-blue duration-200  ">{team.name}</Link>
                    </div>
